...
function handleUpload(files, index) {
    props.form.documents_insurance_policies[index].files = files;
}

function handleRemoveFile(parentIndex, fileIndex) {
    props.form.documents_insurance_policies[parentIndex].files.splice(fileIndex, 1);
}
</script>

<template>
    <div class="bg-white shadow sm:rounded-2xl p-6">
        <p class="text-xl font-semibold">Insurance Policies</p>
        <div class="border-t mt-5">
            <div v-for="(document, index) in form.documents_insurance_policies" class="mt-5">
                <div>
