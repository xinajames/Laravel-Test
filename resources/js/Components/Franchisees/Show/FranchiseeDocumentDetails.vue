...
function goToDocumentsTab() {
    router.visit(route('franchisees.show', { franchisee: props.franchisee.id, tab: 'Documents' }));
}
</script>
...
<template>
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-6">
        ...
        <div
            v-if="franchisee.recent_documents && franchisee.recent_documents.length > 0"
    ...
    >
        <div
            v-for="document in franchisee.recent_documents"
            :key="document.id"
