...
function handleUpload(files, index) {
    props.form.documents_legal_agreements_special[index].files = files;
}

function handleRemoveFile(parentIndex, fileIndex) {
    props.form.documents_legal_agreements_special[parentIndex].files.splice(fileIndex, 1);
}
</script>

<template>
    <div class="bg-white shadow sm:rounded-2xl p-6">
        <p class="text-xl font-semibold">Legal Agreements and Special Documents</p>
        <div class="border-t mt-5 space-y-5">
            <div v-for="(document, index) in form.documents_legal_agreements_special" class="mt-5">
                <div>
