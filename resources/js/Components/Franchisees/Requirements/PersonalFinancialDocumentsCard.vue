...
function handleUpload(files, index) {
    props.form.documents_personal_financial[index].files = files;
}

function handleRemoveFile(parentIndex, fileIndex) {
    props.form.documents_personal_financial[parentIndex].files.splice(fileIndex, 1);
}
</script>

<template>
    <div class="bg-white shadow sm:rounded-2xl p-6">
        <p class="text-xl font-semibold">Personal and Financial Documents</p>
        <div class="border-t mt-5">
            <div v-for="(document, index) in form.documents_personal_financial" class="mt-5">
                <div>
