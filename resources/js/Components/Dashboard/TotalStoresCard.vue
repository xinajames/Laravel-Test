...
const chartData = computed(() => {
    const labels = ['Luzon', 'Visayas', 'Mindanao'];
    ...
    const datasets = [
        {
            ...
            data: labels.map(
                (g) => props.stores.find((s) => s.group === g)?.storeType?.outlet ?? 0
            ),
        ...
        },
        {
            ...
            data: labels.map(
                (g) => props.stores.find((s) => s.group === g)?.storeType?.junior ?? 0
            ),
            borderRadius: radiusForTop,
            borderSkipped: 'bottom',
        },
        {
            ...
            data: labels.map(
                (g) => props.stores.find((s) => s.group === g)?.storeType?.express ?? 0
            ),
            borderRadius: radiusForTop,
            borderSkipped: 'bottom',
        },
        {
            ...
            data: labels.map(
                (g) => props.stores.find((s) => s.group === g)?.storeType?.branch ?? 0
            ),
            borderRadius: radiusForTop,
            borderSkipped: 'bottom',
        },
...
const maxValue = computed(() => Math.max(...chartData.value.datasets.flatMap((d) => d.data), 0));
...
<template>
    ...
    <DashboardInformationModal :open="showInfo" type="total-stores" @close="showInfo = false" />
</template>
