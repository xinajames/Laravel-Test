...
use App\Services\Royalty\MNSRService;
...
class GenerateMnsrJob implements ShouldQueue
{
        ...
        try {
            ...
            foreach ($macroUploads as $macroUpload) {
                try {
                    ...
                    $mnsrService->createMNSR($this->batch_id, $month, $year);
                } catch (Exception $serviceException) {
                    // Log the service-level error to MacroBatch
                    $this->logErrorToMacroBatch(
                        $this->batch_id,
                        $serviceException,
                        "GenerateMnsrJob::handle - MNSR creation failed for file: {$macroUpload->file_name}",
                        'critical'
                    );
    ...
    {
        $this->markBatchAsFailedWithError($this->batch_id, $e, 'GenerateMnsrJob failed');
    }
}
